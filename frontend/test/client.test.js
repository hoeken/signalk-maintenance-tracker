...
import { apiFetch, buildQuery, ApiError, API_BASE } from '../../public/app/api/client.js';
import { loginModalOpen, authState } from '../../public/app/auth/auth.js';
import { mockFetch } from './helpers.js';

describe('buildQuery', () => {
  it('skips empty values and prefixes ?', () => {
    expect(buildQuery({ search: 'oil', page: 2, tags: '', sort: undefined })).toBe('?search=oil&page=2');
...
describe('apiFetch', () => {
  it('prefixes the API base and parses JSON', async () => {
    const fn = mockFetch([{ match: (m, u) => u.indexOf('/api/tasks') !== -1, body: { data: [] } }]);
  ...
  it('throws ApiError with the server error shape', async () => {
    mockFetch([{ match: () => true, status: 404, body: { error: { code: 'not_found', message: 'no such task' } } }]);
  ...
  it('marks the session logged out and opens the login modal on 401 (§7.6)', async () => {
    ...
    mockFetch([{ match: () => true, status: 401, body: { error: { code: 'unauthorized', message: 'nope' } } }]);
