...
describe('sanitizeHtml (§7.8)', () => {
  it('keeps benign markup', () => {
    expect(sanitizeHtml('<p>Use <strong>15W-40</strong></p>')).toBe('<p>Use <strong>15W-40</strong></p>');
  });

  it('removes script and style elements', () => {
    expect(sanitizeHtml('<p>hi</p><script>alert(1)</script><style>*{}</style>')).toBe('<p>hi</p>');
  });

  it('removes nested blocked elements', () => {
    expect(sanitizeHtml('<div><em>ok</em><script>alert(1)</script></div>')).toBe('<div><em>ok</em></div>');
  });
